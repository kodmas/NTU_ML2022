...
alpha = 0.8/255/std
def ifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=25):
    x_adv = x
...
class ensembleNet(nn.Module):
    def __init__(self, model_names):
        super().__init__()
        self.models = nn.ModuleList([ptcv_get_model(name, pretrained=True).to(device) for name in model_names])
...
model_names = [
    'resnext29_32x4d_cifar10',
               'preresnet164bn_cifar10','seresnet110_cifar10','sepreresnet110_cifar10',
               'diaresnet56_cifar10','resnet1202_cifar10',
               'resnet56_cifar10','resnet110_cifar10','diapreresnet110_cifar10','pyramidnet272_a200_bn_cifar10',
               'densenet250_k24_bc_cifar10','xdensenet40_2_k36_bc_cifar10','shakeshakeresnet20_2x16d_cifar10']
ensemble_model = ensembleNet(model_names).to(device)
ensemble_model.eval()

adv_examples, ensifgsm_acc, ensifgsm_loss = gen_adv_examples(ensemble_model, adv_loader, ifgsm, loss_fn)
print(f'ensifgsm_acc = {ensifgsm_acc:.5f}, ensifgsm_loss = {ensifgsm_loss:.5f}')

create_dir(root, 'ens_ifgsm', adv_examples, adv_names)

# Commented out IPython magic to ensure Python compatibility.
# %cd ens_ifgsm
!tar zcvf ../ens_ifgsm.tgz *
# %cd ..

adv_examples, ensmifgsm_acc, ensmifgsm_loss = gen_adv_examples(ensemble_model, adv_loader, mifgsm, loss_fn)
print(f'ensmifgsm_acc = {ensmifgsm_acc:.5f}, ensmifgsm_loss = {ensmifgsm_loss:.5f}')

create_dir(root, 'ens_mifgsm', adv_examples, adv_names)

# Commented out IPython magic to ensure Python compatibility.
# %cd ens_mifgsm
!tar zcvf ../ens_mifgsm.tgz *
# %cd ..
...
for i, cls_name in enumerate(classes):
    ...
    cnt += 1
    plt.subplot(len(classes), 4, cnt)
    im = Image.open(f'./ens_mifgsm/{path}')
    logit = model(transform(im).unsqueeze(0).to(device))[0]
    predict = logit.argmax(-1).item()
    prob = logit.softmax(-1)[predict].item()
