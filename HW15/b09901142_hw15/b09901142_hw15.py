# -*- coding: utf-8 -*-
"""ML2022_HW15_Meta_Learning.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1oKRpCYcU7P6qXHna05Kd9EkG6h1HtxdN
...
class Omniglot(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        t1 = transforms.Compose([transforms.RandomRotation(90),transforms.ToTensor()])
        t2 = transforms.Compose([transforms.RandomHorizontalFlip(),transforms.ToTensor()])
        t3 = transforms.Compose([transforms.ColorJitter(contrast=0.5,hue=0.5),transforms.ToTensor()])
        aug1_imgs = [t1(Image.open(img_file)) for img_file in img_list]
        aug2_imgs = [t2(Image.open(img_file)) for img_file in img_list]
        aug3_imgs = [t3(Image.open(img_file)) for img_file in img_list]
        imgs = imgs + aug1_imgs
        imgs = imgs + aug2_imgs
        imgs = imgs + aug3_imgs
...
def get_meta_batch(meta_batch_size, k_shot, q_query, data_loader, iterator):
    ...
    for _ in range(meta_batch_size):
        ...
        #t = transforms.Compose([transforms.RandomRotation(15),
        #                                     transforms.ToTensor()])
        #aug_data = t(task_data[:,:k_shot]).reshape(-1,1,28,28)
        #train_data = torch.cat((train_data, aug_data), 0)
...
test_inner_train_step = 140 # you can change this
...
# write to csv
with open("output50n.csv", "w") as f:
    f.write(f"id,class\n")
